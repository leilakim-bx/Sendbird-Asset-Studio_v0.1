...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="border-b border-studio-border pb-5 mb-5">
      <p className="text-xs font-semibold text-studio-muted uppercase tracking-wider mb-3">
        {title}
      </p>
      {children}
    </div>
      ...
      return (
        <div className="bg-studio-hover rounded-lg p-3 flex flex-col gap-2">
          <div className="flex items-center justify-between">
            <ToggleGroup
              value={msg.role}
              options={[{ value: "user", label: "User" }, { value: "bot", label: "Bot" }]}
              onChange={(role) => updateMessage(msg.id, { role })}
            />
            <button
              onClick={() => removeMessage(msg.id)}
              className="text-studio-muted hover:text-red-400 text-xs ml-2"
            >
              ✕
            </button>
          </div>
      ...
      return (
        <div className="bg-studio-hover rounded-lg p-3 flex flex-col gap-2">
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => removeMessage(msg.id)} className="text-studio-muted hover:text-red-400 text-xs">✕</button>
          </div>
      ...
      return (
        <div className="bg-studio-hover rounded-lg p-3 flex flex-col gap-2">
          ...
          {msg.items.map((item, i) => (
            <div key={i} className="flex flex-col gap-1 border-t border-studio-border pt-2">
              <Input
                value={item.name}
                onChange={(e) => {
          ...
          <Button
            ...
            onClick={() =>
              addMessage({
                ...
                type: "products",
                items: [
                  { img: "https://picsum.photos/seed/p1/200/240", name: "Product A", sub: "$0.00", cta: "View" },
                  { img: "https://picsum.photos/seed/p2/200/240", name: "Product B", sub: "$0.00", cta: "View" },
                ],
              })
            }
          >
